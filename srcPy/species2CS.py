...
os.makedirs(ddir + f'/{mode}/case_studies', exist_ok=True)
...
for cl in all_sp:
    for sp_dir in tqdm.tqdm(glob.glob(base + f'/{cl}_{mode}/*')):
        ...
        extants = gpd.read_file(sp_dir + '/extant/extant.shp', columns=['sci_name'])
        ...
        del extants
        os.system(f'mv {sp_dir} {ddir}/{mode}/case_studies/CS_{i:03}')
        i += 1

summary = pd.DataFrame(info, columns=['cs','species', 'extant', 'community', 'class'])
summary.to_csv(ddir + f'/species_{mode}.csv', index=False, header=False, mode='a')
